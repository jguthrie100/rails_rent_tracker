class HousesController < ApplicationController
  ...
  def create
    ...
    if house.save
      redirect_to houses_path, notice: "Added House (#{house.name}) to database"
    else
      redirect_to houses_path, notice: "Failed to add House (#{house.name}) to database: #{house.errors.full_messages.to_sentence}"
    end
  end
